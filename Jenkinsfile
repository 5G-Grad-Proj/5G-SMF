pipeline {
    ...
    stages {
        ...
        stage('Scan Image for Common Vulnerabilities and Exposures') {
            steps {
                sh 'trivy image gradproj/5g-smf --output trivy-report.json'
            }
        }
        stage('Pushing to Dockerhub') {
            steps {
                sh 'docker push gradproj/5g-smf:latest'
            }
        }
    ...
    post {
        always {
            ...
            sh 'docker rmi gradproj/5g-smf'
            sh 'docker rmi gradproj/smf-base'
        }
    }
}